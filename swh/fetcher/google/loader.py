    ...
    def process(self, archive_gs, destination_rootpath):
        ...
        if os.path.exists(filepath_meta):
            ...
            try:
                ...
                meta = r.json()
                self.log.info('Writing %s\'s metadata' % archive_gs)

                print(meta['size'], filename_meta)
        ...
        if not meta:
            raise 'Fail to download metadata, stop.'
        ...
        self.log.debug('Fetching %s\' raw data.' % archive_gs)

        if not os.path.exists(filepath):
            try:
                r = requests.get(meta['mediaLink'], stream=True)
            except Exception as e:
                msg = 'Problem when fetching metadata %s.' % url_meta
                self.log.error(msg)
                raise ValueError(msg, e)
